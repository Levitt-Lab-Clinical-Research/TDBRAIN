...
class dataset:
            ...
            if len(Atrl) > 0:
                for i in range(Atrl.shape[0]):
                    if Atrl[i, 0] == 0:
                        ...
                        artsamples[Atrl[i, 0] - np.int((Atrl[i, 1] - Atrl[i, 0]) * padding):Atrl[i, 1] + np.int(
            ...
            if len(p) > 1:
                ...
                for r in range(n_data_rows):
                    for k in range(ARTtrl.shape[0]):
                        ...
                        EOGweight[k, r, ARTtrl[k, 0]:ARTtrl[k, 1]] = Attaper * Atmpweight[k, r,
                                                                               ARTtrl[k, 0]:ARTtrl[k, 1]]
        ...
        for r in range(n_data_rows):
            if ~np.isnan(self.data[r, 0]):
                for w in range(len(winstarts)):
                    kurt[r, np.int(winstarts[w]):np.int(winends[w])] = kurtosis(
                        self.data[r, np.int(winstarts[w]):np.int(winends[w])], fisher=True)
    ...
    def detect_extremevoltswing(self, threshold=120, padding=0.05, overlap=0.05, winlen=0.5):
        ...
        for r in range(n_data_rows):
            if ~np.isnan(self.data[r, 0]):
                for w in range(len(winstarts)):
                    swing[r, np.int(winstarts[w]):np.int(winends[w])] = np.nanmax(
                        self.data[r, np.int(winstarts[w]):np.int(winends[w])]) - np.nanmin(
                        self.data[r, np.int(winstarts[w]):np.int(winends[w])])
